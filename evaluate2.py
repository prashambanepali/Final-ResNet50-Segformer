...
def compute_seg_metrics(preds, targets, threshold=0.5):
    pn = preds.detach().cpu().numpy().flatten()
    tn = targets.detach().cpu().numpy().flatten().astype(int)
    ...
    acc  = (tp+tnn) / (tp+tnn+fp+fn + 1e-8)

    try:    auc = roc_auc_score(tn, pn)
    except: auc = 0.0
...
def plot_roc_curve(all_preds, all_masks, epoch, save_dir, split="val"):
    pn = all_preds.numpy().flatten()
    tn = all_masks.numpy().flatten().astype(int)
    try:
        fpr, tpr, _ = roc_curve(tn, pn)
        auc         = roc_auc_score(tn, pn)
    except:
